...
export default function Gallery() {
  ...
  return (
    <section className="section bg-surface" id="gallery" style={{ borderTop: '1px solid var(--border-subtle)', borderBottom: '1px solid var(--border-subtle)', direction: dir }}>
      <div className="container">
        ...
        <div className="grid grid-3 gap-6" style={{ marginBottom: '4rem' }}>
          {galleryImages.map((img, idx) => (
            <div key={idx} className="relative overflow-hidden card" style={{ height: '300px', borderRadius: 'var(--radius-md)' }}>
              ...
              <div className="gallery-overlay" style={{
                ...
                background: 'linear-gradient(to top, rgba(13,13,13,0.95) 0%, rgba(13,13,13,0.4) 60%, rgba(13,13,13,0.2) 100%)',
                opacity: 0.85, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end',
                padding: '2rem 1.5rem', transition: 'all 0.4s'
              }}>
                <span style={{ fontSize: '0.65rem', fontWeight: 600, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--gold)' }}>
                  {img.category}
                </span>
                <h3 style={{ fontSize: '1.2rem', color: 'var(--text-primary)', marginTop: '0.25rem', fontWeight: 500 }}>
                  {img.title}
                </h3>
              </div>
        ...
        {/* CTA Bar */}
        <div className="text-center" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', padding: '2.5rem 2rem', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-gold)' }}>
          <h3 style={{ fontSize: '1.5rem', color: 'var(--text-primary)', marginBottom: '0.5rem', fontWeight: 600 }}>
            {t('gallery.cta.title')}
          </h3>
